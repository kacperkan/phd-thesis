  ...
  \begin{subfigure}[c]{\linewidth}
    ...
    \resizebox{\linewidth}{!}{
      \begin{tikzpicture}[
        ...
        \node[above right=2em and 0em of pictures.south east, anchor=west, scale=0.85] (plot) {\input{assets/\clogdirname/teaser/teaser_dtu.pgf}};
...
\newcommand{\clogteaserfigure}{
  \begin{figure}
    ...
    \centering
    \clogteasermanual
    \caption[Teaser introducing \clog]{
      \textbf{Teaser} -- We present a novel method to introduce continuous levels of detail (LoD) for Gaussian Splatting.
      We represent Gaussians as a set of vectors~$\cloglatents$ embedded onto a 2D
      grid, which we can easily subsample to obtain a desired level of detail.
      To account for any artifacts that can occur during downsampling, we train a
      modulator~$\modulator$ that corrects such errors.
